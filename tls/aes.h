...
public:
	...
	void print_key();//print 11 round keys
	void iv(const unsigned char *key);
	void encrypt(unsigned char *m) const;
	void decrypt(unsigned char *m) const;
	void encrypt(unsigned char *p, int sz) const;//sz should be multiple of 16
	void decrypt(unsigned char *p, int sz) const;
	template<class It> std::vector<unsigned char> encrypt(It begin, It end) {
		std::vector<unsigned char> v{begin, end};
		encrypt(&v[0], v.size());
		return v;
	}
	template<class It> std::vector<unsigned char> decrypt(It begin, It end) {
		std::vector<unsigned char> v{begin, end};
		decrypt(&v[0], v.size());
		return v;
	}
...
private:
	...
	static uint8_t precomputed[4][256];//galois field 9,11,13,14 computed value
